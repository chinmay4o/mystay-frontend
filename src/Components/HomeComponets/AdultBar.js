  ...
  return (
    <div className="dropdown bg-white ">
      ...
      >
        {roomAdults.map((roomAdult, index) => (
          <div className="flex  gap-4 items-center" key={index}>
            ...
            <div className="flex gap-2 items-center justify-center rounded-full bg-secondary p-2">
              ...
              {rooms > 1  && 
            <div className="p-2 flex items-center justify-center cursor-pointer bg-secondary  rounded-full stroke-primary hover:bg-white" onClick={()=> handleDecreaseRoom(index)}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
        ...
        <div className="flex gap-4 items-center w-full text-sm justify-center cursor-pointer text-primary" onClick={()=>handleIncreaseRoom()}>Add room</div>
      </div>
    </div>
  );
};
