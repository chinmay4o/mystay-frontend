    ...
    return (
      <>
        <div className="h-screen w-screen relative">
          <img src={allSearchedHotels.data.city.image} alt="" className="brightness-75 h-full w-full object-cover"/>
        </div>
          <h1 className="absolute text-6xl -translate-x-1/2 translate-y-1/2 font-bold top-1/2 left-1/2 text-white">{allSearchedHotels.data.city.name.toUpperCase()}</h1>

        
        <div className="w-full min-h-[354px] p-5 bg-secondary">
          <h1 className="text-center text-3xl text-primary capitalize font-semibold mt-6">Stays At {allSearchedHotels.data.city.name}</h1>
